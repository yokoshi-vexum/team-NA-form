...
function buildEveningBlocks(d) {
  ...
  const blocks = [
    ...
    {
      type: 'context',
      elements: [{ type: 'mrkdwn', text: `担当: ${userDisplay(d.submitterName)}　日付: ${today}` }],
    },
  ];
  ...
  blocks.push({
    type: 'section',
    text: { type: 'mrkdwn', text: `*2. お客さんと何か話した？*\n　${d.clientInteraction || '特になし'}` },
  });
  blocks.push({
    type: 'section',
    text: { type: 'mrkdwn', text: `*3. 次は誰が何をやる？*\n　${d.nextAction}` },
  });
  if (d.hitokoto) {
    blocks.push({
      type: 'section',
      text: { type: 'mrkdwn', text: `*4. 今日のひとこと*\n　${d.hitokoto}` },
    });
  }
  ...
  if (d.weeklyMtg === 'yes') {
    ...
    blocks.push({
      type: 'section',
      text: { type: 'mrkdwn', text: `📅 *【週次MTG】* 希望あり${agenda}` },
    });
  } else {
    blocks.push({
      type: 'section',
  ...
  const needs = d.jochiNeeds || [];
  if (needs.length > 0) {
    const content = d.jochiContent ? `\n　内容: ${d.jochiContent}` : '';
    blocks.push({
      type: 'section',
      text: { type: 'mrkdwn', text: `🏢 *【次の常駐】* ヒアリング・打合せ必要 ✅${content}` },
    });
  }
...
function buildDesignBlocks(d) {
  ...
  const blocks = [
    ...
    { type: 'section', text: { type: 'mrkdwn', text: `*■ゴール（完成の定義）*\n${d.goal}` } },
    {
      type: 'section',
      text: {
        type: 'mrkdwn',
  ...
  if (d.failure) {
    blocks.push({ type: 'section', text: { type: 'mrkdwn', text: `*■もしもの時*\n${d.failure}` } });
  }
...
function createServer(slackApp) {
  const app = express();
